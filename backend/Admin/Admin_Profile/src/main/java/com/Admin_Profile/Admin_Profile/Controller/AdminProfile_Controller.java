...
public class AdminProfile_Controller {
    ...
    public ResponseEntity<List<AdminProfile_Entity>> getAllProfiles() {
        return ResponseEntity.ok(profileService.getAllProfiles());
    }

    // ──────────── GET BY REGISTER NUMBER ────────────
    @GetMapping("/by-reg/{regNo}")
    public ResponseEntity<AdminProfile_Entity> getByRegNo(@PathVariable String regNo) {
        Optional<AdminProfile_Entity> profile = profileService.getProfileByRegNo(regNo);
        return profile.map(ResponseEntity::ok)
                      .orElse(ResponseEntity.notFound().build());
    }

    // ──────────── SAVE / UPDATE BY REGISTER NUMBER (upsert) ────────────
    @PutMapping(value = "/by-reg/{regNo}", consumes = MediaType.MULTIPART_FORM_DATA_VALUE)
    public ResponseEntity<AdminProfile_Entity> saveOrUpdateByRegNo(
            ...
            @RequestParam(value = "image", required = false) MultipartFile image) {
        try {
            AdminProfile_Entity saved = profileService.saveOrUpdateByRegNo(
                regNo, registerNo, name, email, phone, section, image);
